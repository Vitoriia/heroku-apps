...
const execute = async () => {
    await fs.writeFile(path.join(__dirname, 'my-write.txt'),'Hello', 'utf-8');
    const data = await fs.readFile(path.join(__dirname, 'my-write.txt'), 'utf-8');
    console.log(data);
    await fs.unlink(path.join(__dirname, 'my-write.txt'));
}
